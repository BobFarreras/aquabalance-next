// src/components/contacte/ContactForm.jsx
"use client";

import React, { useState } from 'react';
import Link from 'next/link'; // Canviem l'import
...
const ContactForm = () => {
  ...
  const [formData, setFormData] = useState({ name: '', email: '', phone: '', subject: '', message: '', productInterest: '' });
  ...
  const handleSubmit = (e) => {
    ...
    if (!privacyAccepted) {
      toast({
        ...
        description: "Has d'acceptar la política de privacitat.",
        variant: "destructive",
      });
      return;
    }
    ...
    // ✅ Llegim les variables d'entorn amb 'process.env'
    const serviceId = process.env.NEXT_PUBLIC_EMAILJS_SERVICE_ID;
    const templateId = process.env.NEXT_PUBLIC_EMAILJS_TEMPLATE_ID;
    const publicKey = process.env.NEXT_PUBLIC_EMAILJS_PUBLIC_KEY;
    
    // La resta de la lògica d'EmailJS es manté igual
    emailjs.send(serviceId, templateId, formData, publicKey)
      .then(() => {
        toast({ title: t('contactPage.form.toast.title'), description: t('contactPage.form.toast.description') });
        setFormData({ name: '', email: '', phone: '', subject: '', message: '', productInterest: '' });
        setPrivacyAccepted(false);
      })
      .catch((err) => {
        console.error('FAILED...', err);
        toast({ title: "Error en l'enviament", description: "Si us plau, intenta-ho més tard.", variant: "destructive" });
      })
      .finally(() => {
  ...
  return (
    <div className="bg-white rounded-2xl p-8 shadow-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* ... camps del formulari (name, email, phone, etc.) ... */}
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="text-sm">
            <label htmlFor="privacy" className="text-gray-700">
              ...
              {/* ✅ Utilitzem el Link de Next.js */}
              <Link href="/politica-de-privacitat" target="_blank" rel="noopener noreferrer" className="font-semibold text-sky-600 hover:underline">
