          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16 items-center">
            ...
            >
              ...
              <div className="space-y-8">
                {Array.isArray(healthBenefits) && healthBenefits.map((benefit, index) => (
                  <div key={index} className="flex items-start">
                    <div className="flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full water-gradient text-white">
                      {healthIcons[index]}
                    </div>
                    <div className="ml-5">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16 items-center">
            ...
            <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
              ...
              <div className="space-y-8">
                {Array.isArray(beautyBenefits) && beautyBenefits.map((benefit, index) => (
                  <div key={index} className="flex items-start">
                    <div className="flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-gradient-to-r from-pink-500 to-purple-600 text-white">
                      {beautyIcons[index]}
                    </div>
                    <div className="ml-5">
                      <h3 className="text-xl font-semibold text-gray-800">{benefit.title}</h3>
                      ...
                      {benefit.testimonials && (
                        <div className="mt-4 space-y-3 border-l-2 border-pink-200 pl-4">
                          {benefit.testimonials.map((testimonial, tIndex) => (
                            // ✅ KEY AFEGIDA AQUÍ
                            <div key={tIndex}>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16 items-center">
            ...
            <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
              ...
              <div className="space-y-8">
                {Array.isArray(sportsBenefits) && sportsBenefits.map((benefit, index) => (
                  <div key={index} className="flex items-start">
                    <div className="flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-gradient-to-r from-orange-500 to-red-500 text-white">
                      {sportsIcons[index]}
                    </div>
                    <div className="ml-5">
                      <h3 className="text-xl font-semibold text-gray-800">{benefit.title}</h3>
      ...
      <section className="py-20 bg-gradient-to-br from-sky-50 to-emerald-50">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {Array.isArray(targetAudiences) && targetAudiences.map((audience, index) => (
